...
public class Model {
    ...
    public boolean maxTileExists() {
        for (int i = 0; i <= board.size() - 1; i++) {
            for (int k = 0; k <= board.size() - 1; k++) {
                if (board.tile(i, k) != null && board.tile(i, k).value() == this.MAX_PIECE) {
                    return true;
                }
            }
        }
    ...
    public boolean atLeastOneMoveExists() {
        ...
        for (int i = 0; i <= board.size() - 1; i++) {
            for (int k = 0; k <= board.size() - 1; k++) {
                if (i - 1 >= 0) {
                    if (board.tile(i - 1, k) != null && board.tile(i - 1, k).value() == board.tile(i, k).value()) {
                        return true;
                    }
                }
                if (i + 1 <= board.size() - 1) {
                ...
                if (k - 1 >= 0) {
                    if (board.tile(i, k - 1) != null && board.tile(i, k - 1).value() == board.tile(i, k).value()) {
                        return true;
                    }
                }
                if (k + 1 <= board.size() - 1) {
    ...
    public void moveTileUpAsFarAsPossible(int x, int y) {
        ...
        for (int i = y + 1; i <= board.size() - 1; i++) {
            if (board.tile(x, i) == null) {
                targetY = i;
            } else if (board.tile(x, i).value() == currTile.value() && !board.tile(x, i).wasMerged()) {
                targetY = i;
            }
        }
        if (board.tile(x, targetY) != currTile && board.tile(x, targetY) != null) {
            board.move(x, targetY, currTile);
            score = score + 2 * currTile.value();
        } else if (board.tile(x, targetY) != currTile) {
            board.move(x, targetY, currTile);
        }
    }
